...
touch_natural_scrolling_id=`xinput list-props ${touch_id} | \
                      ...
                      | cut -d'(' -f2 | cut -d')' -f1`
                      echo "Natural scrolling ID (touchpad) is ${touch_natural_scrolling_id}"

track_natural_scrolling_id=`xinput list-props ${track_id} | \
                      grep -i "Natural Scrolling Enabled (" \
                      | cut -d'(' -f2 | cut -d')' -f1`
                      echo "Natural scrolling ID (trackpoint) is ${track_natural_scrolling_id}"
...
xinput --set-prop ${track_id} ${track_natural_scrolling_id} 1
